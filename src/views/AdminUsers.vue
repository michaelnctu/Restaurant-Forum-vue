    ...
    <table class="table">
      ...
      <tbody>
        ...
        <tr v-for="user in otherUsers" :key="user.id">
          ...
          <td>
            <button
              @click="updatedAdmin({ userId: user.id, isAdmin: user.isAdmin })"
              type="button"
              class="btn btn-link"
            >
...
export default {
  ...
  data() {
    return {
      otherUsers: [], //array
      currentUser: dummyUser.currentUser, //object
    };
  },
  created() {
    this.fetchUsers();
  ...
  methods: {
    fetchUsers() {
      this.otherUsers = dummyData.users; //array
    },
    updatedAdmin({ userId, isAdmin }) {
      console.log("id", userId);
      console.log("isAdmin", isAdmin);
      this.otherUsers = this.otherUsers.map((user) => {
        if (user.id === userId) {
          return {
            ...user,
            isAdmin: !isAdmin,
